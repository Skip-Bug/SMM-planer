...
Программа читает расписание из Google‑таблицы, публикует тексты и изображения по заданному времени, обрабатывает ошибки, повторяет неудачные публикации и удаляет посты по расписанию или вручную.
...
- Аккаунты в **Telegram** (бот и канал), **VK** (сервисный ключ и группа), **OK.ru** (приложение, токен, секретный ключ)
...
TG Статус / VK Статус / OK Статус Заполняется автоматически: Ожидание, Опубликовано, Ошибка, Удалено, REPLAY.
...
Добавьте в список пользователей email сервисного аккаунта (поле client_email из credentials.json) с правами «Редактор». Без этого оркестратор не сможет обновлять статусы и ID постов.
...
2. **Получите `TG_CHANNEL_ID`**:
   ...
   - Для **приватного канала** получите числовой ID (можно через бота `@get_id_bot` или переслав любое сообщение из канала в `@userinfobot`). ID будет выглядеть как `-1001234567890`
...
▶️ Запуск
На Windows
Подробная инструкция по запуску через bat-файлы (разовый запуск, автоперезапуск, настройка службы) описана в отдельном файле:
👉 RUN_WINDOWS.md
...
source venv/bin/activate
python core.py
Программа начнёт опрашивать таблицу каждые 10 секунд (интервал задаётся константой POLL_INTERVAL в core.py).
...
Оркестратор сбросит статус в Ожидание публикации и немедленно повторит публикацию (даже если дата публикации ещё не наступила).
