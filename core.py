...
from sheet_manager import (batch_update_by_headers, get_field,
...
def publish_tg(bot, channel_id, text, img_path=None):
    ...
    if img_path:
        msg_id = tg_send_image(bot, channel_id, img_path, caption=text)
    else:
        msg_id = tg_send_text(bot, channel_id, text)

    return {
        'TG Статус': STATUS['PUBLISHED'],
        'TG id поста': str(msg_id)
    }
...
def process_row(
    ...
    try:
        ...
        delete_flag = get_field(row, col_idx, 'Удалить').upper() == FLAG_TRUE
        ...
        if should_delete:
            ...
            tg_not_delet = tg_status != STATUS['DELETED']
            if tg_bot and tg_channel and tg_id and tg_not_delet:
                try:
            ...
            vk_not_delet = vk_status != STATUS['DELETED']
            if vk_token and vk_owner_int and vk_id and vk_not_delet:
                try:
        ...
        if tg_bot and tg_channel and tg_flag and tg_not_ready:
            try:
                updates = publish_tg(tg_bot, tg_channel, text, img_path)
        ...
        if vk_token and vk_owner_int and vk_flag and vk_not_ready:
            try:
                updates = publish_vk(vk_token, vk_owner_int, text, img_path)
                batch_update_by_headers(0, row_num, updates)
...
def main():
    ...
    # Преобразование owner_id один раз
    vk_owner_int = None
    if vk_owner_raw:
        try:
    ...
    while True:
        try:
            ...
            for row, row_num in zip(rows, row_numbers):
                process_row(row, row_num, col_idx, now, tg_bot,
                            tg_channel, vk_token, vk_owner_int, ok_enabled)
